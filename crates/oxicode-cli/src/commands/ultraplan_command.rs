...
fn derive_phases(goal: &str) -> Vec<Phase> {
    ...
    if lower.contains("api") || lower.contains("endpoint") || lower.contains("backend") {
        return vec![
            phase("Research & Design", "Define data models, contracts, and auth strategy", "ERD + OpenAPI spec", "0.5d"),
            phase("Database layer", "Create migrations and repository traits", "Migration files, model structs", "1d"),
            phase("API endpoints", "Implement handlers, validation, error mapping", "HTTP handlers + integration tests", "2d"),
            phase("Auth & security", "Add authentication / authorisation middleware", "Auth middleware + security tests", "1d"),
            phase("Documentation & review", "Write API docs, run load tests, address review feedback", "README, postman collection", "0.5d"),
        ];
    }

    // UI / frontend-flavoured goals.
    if lower.contains("ui") || lower.contains("frontend") || lower.contains("component") || lower.contains("page") {
        return vec![
            phase("Design & wireframes", "Define component hierarchy and data flow", "Wireframes / Figma mockups", "0.5d"),
            phase("Component scaffolding", "Create skeleton components with prop types", "Component files + Storybook stories", "1d"),
            phase("Logic & state", "Implement business logic and state management", "Working feature with unit tests", "2d"),
            phase("Styling & accessibility", "Apply design tokens, ARIA labels, responsive layout", "Pixel-perfect, a11y-passing UI", "1d"),
            phase("E2E tests & review", "Write Playwright/Cypress tests and address feedback", "E2E test suite", "0.5d"),
        ];
    }
    ...
    if lower.contains("auth") || lower.contains("login") || lower.contains("oauth") {
        return vec![
            phase("Provider research", "Select OAuth providers, review token flows", "Provider comparison doc", "0.25d"),
            phase("Auth models", "Define user, token, and session models", "DB migrations + model structs", "0.5d"),
            phase("OAuth flow", "Implement redirect, callback, and token exchange", "Working OAuth flow", "1.5d"),
            phase("Session & guards", "Add session storage, middleware guards", "Protected routes, refresh logic", "1d"),
            phase("Tests & hardening", "Unit + integration tests, CSRF/PKCE validation", "Full test coverage", "0.75d"),
        ];
    }

    // Generic fallback — five universal phases.
    vec![
        phase("Discovery & planning", &format!("Research requirements for: {goal}"), "Spec document, task list", "0.5d"),
        phase("Foundation", "Set up scaffolding, configs, and data models", "Skeleton code, migrations", "1d"),
        phase("Core implementation", "Build primary features and business logic", "Working feature set", "2d"),
        phase("Testing & QA", "Write unit, integration, and E2E tests", "Test suite, coverage report", "1d"),
        phase("Polish & release", "Address review feedback, update docs, tag release", "Release-ready build + docs", "0.5d"),
    ]
}
